...
"""
'''==========Solving job shop/flow shop scheduling problem by gentic algorithm in python======='''
...
population_size = int(input('Please input the size of population: ') or 30)  # default value is 20  种群规模
...
best_list, best_obj = [], []
population_list = []
for i in range(population_size):    #生成任务调度序列的随机数量 默认20个 染色体
...
for n in range(num_iteration):
    Tbest_now = 99999999999  #初始化迭代的最佳目标值
    '''-------- crossover --------'''
    parent_list = copy.deepcopy(population_list)  #深拷贝当前的种群，使用copy.deepcopy的目的是为了避免后续对parent_list对后原始list的影响
    offspring_list = copy.deepcopy(population_list)
    S = list(
    np.random.permutation(population_size))  # 随机选择种群中的个体来进行交叉操作

for m in range(int(population_size / 2)):
    crossover_prob = np.random.rand()   #生成一个介于0-1之间是的随机数
